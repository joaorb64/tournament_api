...
for player in gsheet["values"][1:]:
    # 'Nick', 'Org', 'Estado', 'Braacket Link', 'Nome real', 'Twitter', 'Mains'
    while len(player) < 7:
        player.append("")
    ...
    for directory in directories:
        with open('player_data/'+directory+'/data.json', 'r') as player_file:
            ...
            player_csv_writer.writerow([
                ...
                player_data.get("braacket_id"),
                player_data.get("full_name"),
                player_data.get("twitter"),
                ("\n".join([main for main in player_data.get("mains")])) if player_data.get("mains") is not None else ""
            ])
'''
'''
