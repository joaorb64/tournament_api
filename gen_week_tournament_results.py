...
def gen_week_results(game):
    ...
    smashgg_characters = json.loads(requests.get(
        "https://api.smash.gg/characters").text)
    ...
    for tournament in weekTournaments.values():
        ...
        try:
            if tournament.get("provider") != "challonge":
                ...
                numEntrants = resp.get("data", {}).get(
                    "event", {}).get("numEntrants")

                if not numEntrants or numEntrants < 6:
                    continue

                tournament_data = resp.get("data", {}).get(
                ...
                if resp.get("data", {}).get("event", {}).get("state") == "COMPLETED" \
                    ...
                    winner = resp.get("data").get("event").get(
                        "standings").get("nodes")[0]
                    entrantId = winner.get("entrant").get("id")

                    participant = winner.get("entrant", {}).get(
                        "participants", [{}])[0].get("user", {})
                    ...
                    if char_data:
                        char_data = char_data.get(
                            "event").get("sets").get("nodes")
                    else:
                    ...
                    char_usage = {k: v for k, v in sorted(
                        char_usage.items(), key=lambda item: item[1], reverse=True)}
                    ...
                    for char in char_usage.items():
                        char_in_json = next(
                            (c for c in smashgg_characters["entities"]["character"] if c["id"] == char[0]), None)
                    ...
                    if len(char_usage_named.keys()) == 0:
                        if userId is not None and str(userId) in smashgg_cache:
                            char_usage_named = smashgg_cache[str(
                                userId)].get("character_usage")

                    character = None

                    if len(char_usage_named.keys()) > 0:
                        character = sorted(
                            char_usage_named.items(), key=lambda t: t[1])[-1][0]

                    if tournament.get("lat") == None or tournament.get("lng") == None:
                        if tournament.get("country_code"):
                            country = next(
                                (c for c in countries_json if c["iso2"] == tournament.get("country_code")), None)
                            if country:
    ...
    with open('./out/'+game+'/week_tournament_results.json', 'w', encoding="utf-8") as outfile:
